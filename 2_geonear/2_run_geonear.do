cap log close
log using "$log_path/2_run_geonear_log.log", replace
...
forval i = 11/18 {
	...
	save "$temp_path/snfclaims_20`j'.dta", replace
}

/* For each year of LTC Focus, saves 4 separate datasets, one for each of the 
% Medicare benes categories (based on quartiles) */
...
forval i = 11/18 {
	forval k = 1/4 {		
		...
		preserve
			...
			save "$temp_path/ltc_nh_days_cat`k'_20`j'.dta", replace 
		restore
	}
}
...
forval i = 11/18 {
	forval k = 1/4 {
		local j : display %02.0f `i'
		use "$temp_path/snfclaims_20`j'.dta", clear
		geonear id bene_zip_lat bene_zip_long using "$temp_path/ltc_nh_days_cat`k'_20`j'.dta", ///
...
forval i = 11/18 {
	local j : display %02.0f `i'
	append using "$temp_path/snfclaims_20`j'.dta"
}
...
label var mi_to_snf1 "Distance to nearest least-specialized nursing home"
label var mi_to_snf2 "Distance to nearest 2nd-quartile nursing home"
label var mi_to_snf3 "Distance to nearest 3rd-quartile nursing home"
label var mi_to_snf4 "Distance to nearest most-specialized nursing home"
